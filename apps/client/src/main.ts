...
async function finishTourneyRound(moves: MoveLog[], score: number): Promise<void> {
  ...
  try {
    ...
    // Часы держат заход с двух сторон, и отказ снизу («журнал пришёл
    // раньше, чем он игрался») тоже правило, а не поломка: живой игрок
    // упрётся в него разве что с уехавшими часами устройства, и молчать
    // об этом хуже, чем сказать прямо.
    const rules: Record<string, string> = {
      'too-late': 'Заход шёл дольше трёх минут — в зачёт не идёт',
      'too-fast': 'Часы разошлись: заход пришёл раньше, чем сыгран',
    };
    resultSubEl.textContent =
      (error instanceof ApiError ? rules[error.code] : undefined) ?? 'Заход не дошёл до турнира';
  }
}
