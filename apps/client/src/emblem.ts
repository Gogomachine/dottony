...
export function emblemSvg(options: { focused?: boolean; size?: number } = {}): string {
  ...
  const ticks = [0, 90, 180, 270]
    .map(
      (angle) =>
        `<line x1="50" y1="4" x2="50" y2="12" stroke="var(--accent)" stroke-width="5"
           stroke-linecap="round" transform="rotate(${angle} 50 50)"/>`,
    )
    .join('');

  const crosshair = focused
    ? `<g stroke="var(--accent)" stroke-width="4" stroke-linecap="round" opacity="0.9">
  ...
  return `<svg width="${size}" height="${size}" viewBox="0 0 100 100" aria-label="dotoscope">
    ${ticks}
    <circle cx="50" cy="50" r="34" fill="none" stroke="var(--accent)" stroke-width="7"/>
    <circle cx="50" cy="50" r="27" fill="var(--board)"/>
    ${crosshair}
    <circle cx="50" cy="50" r="${focused ? 15 : 13}" fill="var(--accent)"/>
    <path d="M33 38 A 22 22 0 0 1 47 27" fill="none" stroke="var(--control)"
