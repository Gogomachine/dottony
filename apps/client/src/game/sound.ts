...
function deg(index: number): number {
  return ROOT * 2 ** ladderPos(index);
}

/** Где ступень стоит на лестнице, в октавах от нижней ноты. */
function ladderPos(index: number): number {
  const step = Math.max(0, index);
  const octave = Math.floor(step / LADDER.length);
  const degree = LADDER[step % LADDER.length]!;
  return octave + degree / 12;
}

/**
 * Докуда цепочка поднимается на самом деле. Дальше нота не идёт: восемнадцать
 * точек проходили две с лишним октавы за две секунды, и конец такого жеста
 * сидел ровно там, где ухо чувствительнее всего. Две октавы подъёма читаются
 * не хуже четырёх, а резать перестают.
 */
const CHAIN_CAP = 10;

/**
 * За сколько ступеней сверх потолка тембр целиком переходит в круг. Ровно до
 * первого возврата: круг замыкается только тогда, когда стопка октав уже
 * собрана целиком, — иначе возврат слышен провалом, потому что подхватить
 * ушедшую вверх ноту ещё нечем.
 */
const CHAIN_SPREAD = 5;

/** Где стоит центр тяжести звука после потолка. Он больше не двигается. */
const CHAIN_CENTER = ROOT * 2 ** ladderPos(CHAIN_CAP);

/** Ширина окна октавной стопки, октав. Уже — глуше, шире — звонче. */
const CHAIN_SIGMA = 0.9;

/**
 * Нота цепочки. До потолка — обычная лестница. Выше — тот же шаг, но внутри
 * одной октавы: нота идёт вверх, доходит до края и возвращается к началу
 * круга. Сам по себе такой возврат слышен как обрыв, поэтому работает он
 * только вместе с октавной стопкой ниже.
 */
function chainNote(index: number): number {
  const cap = ladderPos(CHAIN_CAP);
  const pos = ladderPos(index);
  if (pos <= cap) return ROOT * 2 ** pos;
  return ROOT * 2 ** (cap + ((pos - cap) % 1));
}

/**
 * Насколько далеко цепочка ушла за потолок: 0 у обычной, 1 у совсем длинной.
 * Через неё же идёт всё остальное, что отличает длинную цепочку от короткой.
 */
 ...
 * У длинной — стопка октав, взвешенная неподвижным окном: пока нота идёт
 * вверх по кругу, её собственная октава тише, а нижняя громче ровно
 * настолько, чтобы в момент возврата стопка совпала сама с собой. Ухо слышит
 * подъём на каждом шаге, а частота при этом стоит на месте — это иллюзия
 * Шепарда, и цепочке в ней можно расти сколько угодно.
 */
function chainPartials(freq: number, over: number): [number, number][] {
  if (over <= 0) return PARTIALS;
  const weigh = (ratio: number): number =>
    Math.exp(-(Math.log2((freq * ratio) / CHAIN_CENTER) ** 2) / (2 * CHAIN_SIGMA ** 2));
  const mixed: [number, number][] = [
    [0.25, weigh(0.25) * over],
    [0.5, weigh(0.5) * over],
    [1, 1 - over + weigh(1) * over],
    [2, PARTIALS[1]![1] * (1 - over) + weigh(2) * over],
    // Третья кратная — единственная неоктавная: она и ломала бы совпадение
    // стопки с самой собой, поэтому к длинной цепочке её не остаётся.
    [3, PARTIALS[2]![1] * (1 - over)],
    [4, weigh(4) * over],
  ];
  return mixed.filter(([, weight]) => weight > 0.004);
}
...
export class Sound {
   ...
   *
   * До потолка это обычная нота лестницы. Дальше высота идёт по кругу, а
   * рост цепочки слышен всем остальным сразу: тело тяжелеет нижними
   * октавами, щелчок уходит, фильтр закрывается, звук длиннее и шире по
   * панораме. Длинная цепочка становится не выше короткой, а плотнее.
   */
  step(index: number, at?: number): void {
    const t = this.when(at);
    const over = chainOver(index);
    const dur = 0.19 + 0.07 * over;
    ...
    const pan = Math.max(-0.5, Math.min(0.5, ((index % 7) - 3) * 0.16)) * (1 + 0.5 * over);
    const freq = chainNote(index);
    const partials = chainPartials(freq, over);
    const sum = partials.reduce((total, [, weight]) => total + weight, 0);
    // Щелчок к концу длинной цепочки почти уходит: он весь живёт выше двух
    // килогерц, и это он там режет, а не сама нота.
    this.click(t, 2400, 0.005, level * 0.35 * (1 - 0.75 * over), { q: 0.8, pan, high: true });
    this.body(t, freq, dur, (level * PARTIAL_SUM) / sum, {
      cutoff: 2800 - 1000 * over,
      open: 5200 - 2200 * over,
      pan,
      partials,
    });
