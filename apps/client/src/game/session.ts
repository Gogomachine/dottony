...
/**
 * Длительность матча до ответа сервера. Настоящую всегда говорит он — своим
 * часам в дуэли веры нет, — и у механик она разная: цепочки полторы минуты,
 * заказы три. Здесь стоит цепочечная: режим `duel` — это они и есть.
 *
 * Значение повторяет DUEL_SECONDS.chain протокола; взять его оттуда нельзя:
 * любой не-type импорт из @doton/protocol затащил бы в клиент ещё и zod.
 */
export const DUEL_FALLBACK = 90;
