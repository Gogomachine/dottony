...
export class Tutorial {
  ...
  private async modesAct(): Promise<void> {
    ...
    this.say(
      6,
      'Заказ',
      'Прибор звенит цветом 18 секунд. Снять 25 точек этого цвета за одно касание — размер группы задаёт поле, а не палец. Три пустых окна — заход окончен.',
    );
    await this.wait(2.8);
    ...
    this.say(
      6,
      'Дуэль',
      'Полторы минуты на одинаковом образце с живым соперником. Рейтинг, лиги и заявка на цвет — одна на двоих.',
    );
    await this.wait(2.8);
    this.hud.hideVersus();
  }
