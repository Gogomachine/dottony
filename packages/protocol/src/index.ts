...
/**
 * Строка списка «что заметил прибор»: заход, не похожий на человеческий.
 *
 * Это не обвинение и не жалоба: жалобу пишет человек, а тут сервер сам
 * посмотрел на почерк захода и позвал служащего. Решает всё равно служащий —
 * поэтому в строке лежит не приговор, а словами то, что было замечено.
 */
export interface AdminNotice {
  userId: string;
  name: string;
  /** Где заметили: «турнир» или «рекорд». */
  place: string;
  /** Что именно: острота, ровность темпа — словами. */
  detail: string;
  score: number;
  at: string;
  /** Сколько раз прибор замечал этого игрока. Один раз — совпадение. */
  count: number;
}
...
/**
 * Положения тумблеров лаборатории PETRIDOT.
 *
 * Шкала целая, от нуля до тысячи делений: дробей нарочно нет — положение
 * едет в базу, в ответ сервера и в тест, а сравнивать дроби на трёх
 * дорогах значит однажды не совпасть в последнем знаке.
 */
export const PetriDialsRequestSchema = z.object({
  temp: z.number().int().min(0).max(1000),
  humidity: z.number().int().min(0).max(1000),
  medium: z.number().int().min(0).max(1000),
});

/** Кого отправляют в коллекцию: своего и только по номеру. */
export const PetriShelfRequestSchema = z.object({
  id: z.string().uuid(),
});
...
export interface MeResponse {
  name: string;
  /**
   * Новый пропуск, если имя сменилось не рукой игрока: имя лежит в токене, и
   * после смены службой старый токен носит старое имя. Приходит только в тот
   * раз, когда имя разошлось, — клиенту его надо сохранить вместо прежнего.
   */
  token?: string;
  /**
