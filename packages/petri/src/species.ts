...
/**
 * Куда крутить, чтобы стало лучше: `-1` влево, `1` вправо, `0` — уже хорошо.
 *
 * Это единственная подсказка, которую даёт прибор, и она нарочно не
 * называет числа: «теплее» — совет, «поставь 640» — ответ, после которого
 * ухаживать больше не за чем.
 */
export function hintFor(species: Species, dial: Dial, value: number): -1 | 0 | 1 {
  const comfort = comfortOf(species)[dial];
  if (dialFits(comfort, value)) return 0;
  return value < comfort.at ? 1 : -1;
}
